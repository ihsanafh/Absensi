<x-app-layout>
    <x-slot name="header">Edit Akun Guru</x-slot>

    <!-- Header -->
    <div class="flex justify-between items-start mb-stack-lg border-b border-outline-variant/50 pb-4">
        <div>
            <h2 class="font-h3 text-h3 text-on-surface mb-1">Edit Akun Guru</h2>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-stack-lg border border-outline-variant/50">
        <form action="{{ route('admin.guru.update', $guru->id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-x-stack-lg gap-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-col gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="nama_lengkap" name="nama_lengkap" value="{{ old('nama_lengkap', $guru->nama_lengkap) }}" required class="w-full rounded-lg border-outline-variant bg-surface focus:border-primary focus:ring focus:ring-primary/20 text-on-surface placeholder:text-on-surface-variant/50 transition-shadow">
                ...
                <div>
                    ...
                    <input type="text" id="nip" name="nip" value="{{ old('nip', $guru->nip) }}" required class="w-full rounded-lg border-outline-variant bg-surface focus:border-primary focus:ring focus:ring-primary/20 text-on-surface placeholder:text-on-surface-variant/50 transition-shadow font-label-mono">
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $guru->email) }}" class="w-full rounded-lg border-outline-variant bg-surface focus:border-primary focus:ring focus:ring-primary/20 text-on-surface placeholder:text-on-surface-variant/50 transition-shadow">
                ...
                <div>
                    ...
                    <input type="tel" id="no_telepon" name="no_telepon" value="{{ old('no_telepon', $guru->no_telepon) }}" class="w-full rounded-lg border-outline-variant bg-surface focus:border-primary focus:ring focus:ring-primary/20 text-on-surface placeholder:text-on-surface-variant/50 transition-shadow font-label-mono">
            ...
            <div class="flex flex-col gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="username" name="username" value="{{ old('username', $guru->username) }}" required class="w-full rounded-lg border-outline-variant bg-surface focus:border-primary focus:ring focus:ring-primary/20 text-on-surface placeholder:text-on-surface-variant/50 transition-shadow">
                    <x-input-error :messages="$errors->get('username')" class="mt-2 text-error" />
                </div>
                
                <!-- Password -->
                <div x-data="{ show: false }">
                    <label class="block font-body-sm font-medium text-on-surface mb-1" for="password">Password (Opsional)</label>
                    <div class="relative">
                        <input :type="show ? 'text' : 'password'" id="password" name="password" class="w-full rounded-lg border-outline-variant bg-surface focus:border-primary focus:ring focus:ring-primary/20 text-on-surface placeholder:text-on-surface-variant/50 pr-10 transition-shadow" placeholder="Kosongkan jika tidak diubah">
                ...
                <div class="mt-2 flex items-center justify-between p-4 rounded-lg bg-surface-container-low border border-outline-variant/30">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="status_akun" value="aktif" class="sr-only peer" {{ old('status_akun', $guru->status_akun) === 'aktif' ? 'checked' : '' }}>
